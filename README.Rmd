...
probability is converted to a Mahalanobis threshold using the effective rank
...
`rerand_estimate()` supports difference-in-means and Lin's fully interacted
regression adjustment. Use `accept_prob = 1` for complete randomization.
...
Set `theoretical = TRUE` and provide `Y_full = cbind(Y0, Y1)` to calculate
population-level benchmarks alongside the sample estimate.
...
warnings. New code should use `rerand_design()`, `rerand_estimate()`, and
`get_quantile()`.
